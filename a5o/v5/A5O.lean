...
--      look finished.
--  11. Each of the nine axioms below was stated over this file's own
--      section variables (`C`, `R`, `Appointed`, `Revoke`, `SpeaksFor`,
--      `Empowers`, `Copy`, `SignedBlock`, `UCANRevoke`/`R'`), which Lean
--      generalizes automatically — so every axiom asserted its property
--      for EVERY predicate of the matching type, not just the one this
--      file actually reasons about. Independent check, 6 Sep 2026
--      (`Consistency.lean`, reviewer's consistency probe): instantiating
--      each axiom at a deliberately pathological predicate (e.g.
--      `C_functional` at the constantly-true signature predicate on
--      `Bool`) derives `False` from that axiom alone. The axiom set was
--      inconsistent, which makes every axiom-dependent result vacuously
--      true regardless of its proof — a defect in how the axioms were
--      stated, not in the reasoning that used them. Fixed: each axiom is
--      now an explicit hypothesis parameter on the theorem(s) that
--      consume it (`O_unique`; `revoked_defeats_O` and
--      `revocation_blocks_enforcement`; `ABLP.D3_...`;
--      `SPKI.SPKI_threshold_...`; `Macaroons.bearer_tokens_...`;
--      `Biscuit.biscuit_chain_...`; `UCAN.ucan_matches_P2_shape`),
--      applied to the one abstract predicate already in scope rather
--      than asserted of every predicate of that type. `#print axioms`
--      on every theorem in the file now reports zero axioms. Nothing
--      else changes: no definition, no `O` predicate, and no proof
--      strategy is touched, and the results that never depended on any
--      axiom (the necessity theorems `N1_necessity`-`N6_necessity`,
--      `P1`, `P3`, `P4`, `P5`, `U3_reduction`, the completeness-direction
--      theorems, and the pure type-level D1/D2-and-dual facts) are
--      exactly as they were. Two theorems are added as the documented
--      boundary of `O_unique`'s new hypothesis: `quorum_two_governors`
--      exhibits two distinct principals both satisfying `O` on a
--      board-quorum instance of the sealed predicate, and
--      `quorum_not_CFunctional` proves that instance fails exactly the
--      hypothesis `O_unique` now requires — the boundary is on file, not
--      merely asserted.
...
-- Functionality of the signature predicate, as a hypothesis on the one
-- C already in scope, not a universally-quantified axiom over every C
-- of the matching type (see revision 11).
...
-- The documented boundary of O_unique: a board-quorum instance of the
-- sealed O predicate where two distinct principals both satisfy it,
-- shown to fail exactly the hypothesis O_unique now requires.
inductive QuorumPrin | a | b | x deriving DecidableEq
open QuorumPrin

def quorumC : QuorumPrin → Unit → Unit → Unit → Prop := fun p _ _ _ => p = a ∨ p = b
def quorumI : QuorumPrin → Prop := fun _ => True
def quorumM : Unit → Unit → Prop := fun _ _ => True
def quorumS : Unit → Unit → Prop := fun _ _ => True
def quorumR : QuorumPrin → Unit → Unit → Unit → Prop := fun _ _ _ _ => True
def quorumAppointed : QuorumPrin → Unit → Unit → Unit → Prop := fun _ _ _ _ => True

theorem quorum_two_governors :
    O quorumC quorumI quorumM quorumS quorumR quorumAppointed a () () () ∧
    O quorumC quorumI quorumM quorumS quorumR quorumAppointed b () () () ∧
    a ≠ b :=
  ⟨⟨Or.inl rfl, trivial, trivial, trivial, trivial, trivial⟩,
   ⟨Or.inr rfl, trivial, trivial, trivial, trivial, trivial⟩,
   by decide⟩

theorem quorum_not_CFunctional : ¬ CFunctional quorumC :=
  fun h => by
    have := h a b () () () (Or.inl rfl) (Or.inr rfl)
    cases this
...
-- ---- P2 (revocation dominance — now a hypothesis, not an axiom) -------
--
-- P2: the binding must be revocable by the principal alone. Stated as a
-- hypothesis on the theorems below that consume it (see revision 11),
-- not as a standalone axiom universally quantified over every R.
...
-- D3: ABLP's own compound-principal delegation rule, taken as hypotheses
-- (standard content of the calculus, not something this file invents)
-- on the theorem that uses them. Both conjuncts of `Conj A B` speak for
-- the compound.
--
...
-- axiom and onto `CFunctional` earlier in this file.
theorem D3_compound_principals_violate_single_governor
    (hLeft : ∀ (A B : Prin), SpeaksFor A (Conj A B))
    (hRight : ∀ (A B : Prin), SpeaksFor B (Conj A B))
    (A B : Prin) (hAB : A ≠ B) :
    SpeaksFor A (Conj A B) ∧ SpeaksFor B (Conj A B) ∧ A ≠ B :=
  ⟨hLeft A B, hRight A B, hAB⟩
...
theorem SPKI_threshold_violates_single_governor
    (hLeft : ∀ (A B : Prin), Empowers A (Threshold1of2 A B))
    (hRight : ∀ (A B : Prin), Empowers B (Threshold1of2 A B))
    (A B : Prin) (hAB : A ≠ B) :
...
-- `CFunctional`-style constraint on how many principals can satisfy
...
-- taken as a hypothesis on the theorem that uses it.
theorem bearer_tokens_admit_multiple_governors
    (hCopyPreserves : ∀ (P Q : Prin), Authorized P → Copy P Q → Authorized Q)
    (A B : Prin) (hA : Authorized A) (hCopy : Copy A B) (hAB : A ≠ B) :
    Authorized A ∧ Authorized B ∧ A ≠ B :=
  ⟨hA, hCopyPreserves A B hA hCopy, hAB⟩
...
-- the delegation chain — the same shape as this file's `CFunctional`.
...
theorem biscuit_chain_integrity_matches_C_functional
    (hFunctional : ∀ (P Q R : Prin), SignedBlock P R → SignedBlock Q R → P = Q)
    (P Q R : Prin) (h1 : SignedBlock P R) (h2 : SignedBlock Q R) : P = Q :=
  hFunctional P Q R h1 h2
...
theorem ucan_matches_P2_shape
    (hRevDominance : ∀ (Iss Sub : Prin), Appointed' Iss Sub → UCANRevoke Iss Sub → ¬ R' Iss Sub)
    (Iss Sub : Prin) (hApp : Appointed' Iss Sub) (hRev : UCANRevoke Iss Sub) :
    ¬ R' Iss Sub :=
  hRevDominance Iss Sub hApp hRev
...
-- compile error instead of a silent reinterpretation. Zero `axiom`
-- declarations remain in the file (see revision 11): what were nine
-- axioms are now explicit hypothesis parameters —
--   `CFunctional` (as `hC`), the P2 shape (as `hP2`) — this file's own
--   two (see above).
--   `hLeft`/`hRight` on `ABLP.D3_...` — ABLP's compound-principal rule.
--   `hLeft`/`hRight` on `SPKI.SPKI_threshold_...` — RFC 2693's threshold subject.
--   `hCopyPreserves` on `Macaroons.bearer_tokens_...` — the documented bearer property.
--   `hFunctional` on `Biscuit.biscuit_chain_...` — Ed25519 chain-signature unforgeability.
--   `hRevDominance` on `UCAN.ucan_matches_P2_shape` — UCAN's own issuer-revocation mechanism.
-- `#print axioms` on every theorem in this file — including
-- `ABLP.D1_no_revocation_dominance_primitive`, `ABLP.D2_not_act_specific`,
-- `SPKI.spki_certs_can_vary_over_time`, `SPKI.spki_certs_can_vary_over_action`,
-- O_unique, P1, P3, P4, P5, N1-N6, U3_reduction, every D1_.../D2_..., the
-- Biscuit/UCAN correspondence theorems, the four completeness-direction
-- theorems (`canonical_monitor_achieves_ADA`, `canonical_monitor_is_complete`,
-- `overcautious_monitor_achieves_ADA`, `completeness_is_not_universal`),
-- and `quorum_two_governors`/`quorum_not_CFunctional` — now reports zero
-- axioms. Nothing in this list changed its proof strategy; each is a
-- proof, not a postulate, exactly as before.
...
-- result in the file that actually consumes the revocation-dominance
-- hypothesis.
