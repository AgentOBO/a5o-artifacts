...
/-! ══════════════════════════════════════════════════════════════════════════
  ## 5. Framing — adopted by the Chairman, 2026-09-19.
  F4. The Clock Standard is a conversion table between geography and
      accountability.  Every placement of the revocation snapshot relative
      to the gate has an exact exposure window in ticks, and that window is
      what the receipt's `revocation_as_of` is honest about.  A deployment
      attestation can therefore state a number — "snapshot in L2, exposure
      ≤ 1 tick"; "snapshot across the hall, exposure ≤ 28 ticks" — and a
      third party checks it against physics, not against the vendor.
      Propagation delay bounding freshness is ordinary physics; the table
      exists only because the tick is fixed and public.
  ══════════════════════════════════════════════════════════════════════════ -/

/-- F4, as the four rows of the table that matter. -/
theorem F4_table :
    ps_to_ticks_ceil L2_ps = 1 ∧
    ticks_one_way 600000 um_per_s_copper = 1 ∧
    ticks_one_way 100000000 um_per_s_fibre = 28 ∧
    ps_to_ticks_ceil (10 ^ 9) = 57143 := by
  decide
